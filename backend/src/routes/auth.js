...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const user = await User.create({
      ...
      role: requestedRole,
      specialties: requestedRole === 'agente' ? (req.body.specialties || []) : []
    });
    ...
    res.status(201).json({
      token,
      user: { id: user.id, name: user.name, email: user.email, role: user.role, specialties: user.specialties }
    });
  } catch (error) {
...
// REGISTRO DE USUARIOS POR EL ADMINISTRADOR (Ruta Protegida, solo Sysadmin)
router.post('/register-by-admin', protect, authorize('sysadmin'), async (req, res) => {
